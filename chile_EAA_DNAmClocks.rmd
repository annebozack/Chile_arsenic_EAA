...
setwd('Chile_arsenic_epigenetic_aging')
...
all_pheno = read.sas7bdat('AC13_052018.sas7bdat')
...
write.csv(PBMC_pheno, file = 'PBMC_fun_pheno_withAges.csv')
```
...
fp_PBMCs_robust = ggplot(data=df, aes(x=label, y=mean, ymin=lower, ymax=upper)) +
  ...
  ylim(-3, 11)
quartz.save(file = 'PBMC_ageAccelAssoc_forestPlots_robust.png', type = 'png', dpi = 300)

# Raincloud plots
marker = NULL
y_axis = NULL
x_axis = NULL
for(i in 1:nrow(PBMC_dfAge)){
    marker = c(marker, rep(rownames(PBMC_dfAge)[i], times = nrow(PBMC_pheno)))
    y_axis = c(y_axis, PBMC_pheno[[rownames(PBMC_dfAge)[i]]])
    x_axis = c(x_axis, PBMC_pheno$exposed)
}
dfBox = data.frame(marker, y_axis, x_axis)

for (i in 1:nrow(PBMC_dfAge)){
    df_1x1 <- data_1x1(array_1 = dfBox$y_axis[dfBox$x_axis == 1 & dfBox$marker == rownames(PBMC_dfAge)[i]], 
      array_2 = dfBox$y_axis[dfBox$x_axis == 2 & dfBox$marker == rownames(PBMC_dfAge)[i]], jit_distance = .05, jit_seed = 321)
    df_1x1$jit = df_1x1$jit - 0.15
    assign(paste0("plot_", rownames(PBMC_dfAge)[i]), raincloud_1x1(data = df_1x1,  
      colors = (c('black','black')),  
      fills = (c('#7E7E7E','#2F2F2F')),  
      size = 0.5,  
      alpha = .6,  
      ort = 'v') + scale_x_continuous(breaks=c(1.25,2.25), labels=c("Not exposed", "Exposed"), limits=c(0.5, 3)) +
  xlab("") + 
  ylab(rownames(PBMC_dfAge)[i]) +
  theme_classic())
}

grid.arrange(plot_AgeAccelerationResidual, plot_AgeAccelerationResidualHannum, plot_AgeAccelPheno, plot_DNAmAgeSkinBloodClockAdjAge, plot_AgeAccelGrim, plot_IEAA, plot_EEAA, plot_DNAmTLAdjAge, nrow = 3) 
quartz.save(file = 'PBMC_ageAccel_rainCloudPlots.png', type = 'png', dpi = 300)

# plot phenoAge only
i = 3
dfBox$x_axis = factor(dfBox$x_axis, levels = c(2,1)) # reverse order of exposed and unexposed
df_1x1 <- data_1x1(array_1 = dfBox$y_axis[dfBox$x_axis == 2 & dfBox$marker == rownames(PBMC_dfAge)[i]], 
      array_2 = dfBox$y_axis[dfBox$x_axis == 1 & dfBox$marker == rownames(PBMC_dfAge)[i]], jit_distance = .07, jit_seed = 900)
    df_1x1$jit = df_1x1$jit - 0.17
    raincloud_1x1(data = df_1x1,  
      colors = (c('black','black')),  
      fills = (c('#3B7EA1','#FDB515')),  
      size = 0.6,  
      alpha = .9,  
      ort = 'v') + scale_x_continuous(breaks=c(1.25,2.25), labels=c("Exposed", "Unexposed"), limits=c(0.7, 3)) +
  xlab("") + 
  ylab('PhenoAge acceleration (years)') +
  theme_classic()
quartz.save(file = 'PBMC_PhenoAgeAccel_rainCloudPlot.png', type = 'png', dpi = 300)

```

#### Adjusted robust linear regression results
```{r, echo = F, out.width = '75%'}
knitr::include_graphics("PBMC_ageAccelAssoc_forestPlots_robust.png")
```

#### Raincloud plots
```{r, echo = F, out.width = '100%'}
knitr::include_graphics("PBMC_ageAccel_rainCloudPlots.png")
```
...
write.csv(buccalCell_pheno, 'buccalCell_fun_pheno_withAges.csv')
```
...
# 3.889267
scatter_DNAmAgeHannum_buccalCell = ggplot(buccalCell_pheno, aes(age, DNAmAgeHannum)) + geom_smooth(method = 'lm', alpha = 0.2, size = 0.5, color = 'black') + geom_point(size = 0.75) + theme_classic() + labs(x = 'Chronological age (years)', y = 'Hannum DNAm age') + coord_cartesian(ylim = c(36,62), clip = 'off') + theme(axis.title.x = element_blank()) + labs(title = expression(paste('r = 0.50, ', italic('p'), ' = 0.001, ', 'MAE = 3.89'))) + theme(plot.title = element_text(size=8.5)) + theme(text = element_text(size = 8)) + theme(plot.title = element_text(hjust = 0.5))
...
# 2.868919
scatter_DNAmAgeSkinBloodClock_buccalCell = ggplot(buccalCell_pheno, aes(age, DNAmAgeSkinBloodClock)) + geom_smooth(method = 'lm', alpha = 0.2, size = 0.5, color = 'black') + geom_point(size = 0.75) + theme_classic() + labs(x = 'Chronological age (years)', y = 'Skin and blood DNAm age') + coord_cartesian(ylim = c(34,63), clip = 'off') + labs(title = expression(paste('r = 0.72, ', italic('p'), ' < 0.001, ', 'MAE = 2.87')), x = 'Chronological age') + theme(plot.title = element_text(size=8.5)) + theme(text = element_text(size = 8)) + theme(plot.title = element_text(hjust = 0.5))
...
```{r, echo = F, out.width = '100%', echo = F}
knitr::include_graphics("buccalCell_clocks_scatterPlots.png")
```
...
fp_PBMCs_robust = ggplot(data=df, aes(x=label, y=mean, ymin=lower, ymax=upper)) +
  ...
  theme(plot.title = element_text(size=8.6, hjust = 0.5),panel.grid.major.x = element_blank(),
        ...
        axis.title.x = element_text(size=8),
        axis.title.y=element_text(size=8.5)) 
quartz.save(file = 'buccalCell_ageAccelAssoc_forestPlots_robust.png', type = 'png', dpi = 300)

# Raincloud plots
marker = NULL
y_axis = NULL
x_axis = NULL
for(i in 1:nrow(buccalCell_dfAge)){
    marker = c(marker, rep(rownames(buccalCell_dfAge)[i], times = nrow(buccalCell_pheno)))
    y_axis = c(y_axis, buccalCell_pheno[[rownames(buccalCell_dfAge)[i]]])
    x_axis = c(x_axis, buccalCell_pheno$exposed)
}
dfBox = data.frame(marker, y_axis, x_axis)

for (i in 1:nrow(buccalCell_dfAge)){
    df_1x1 <- data_1x1(array_1 = dfBox$y_axis[dfBox$x_axis == 1 & dfBox$marker == rownames(buccalCell_dfAge)[i]], 
      array_2 = dfBox$y_axis[dfBox$x_axis == 2 & dfBox$marker == rownames(PBMC_dfAge)[i]], jit_distance = .05, jit_seed = 321)
    df_1x1$jit = df_1x1$jit - 0.15
    assign(paste0("plot_", rownames(buccalCell_dfAge)[i]), raincloud_1x1(data = df_1x1,  
      colors = (c('black','black')),  
      fills = (c('#7E7E7E','#2F2F2F')),  
      size = 0.5,  
      alpha = .6,  
      ort = 'v') + scale_x_continuous(breaks=c(1.25,2.25), labels=c("Not exposed", "Exposed"), limits=c(0.5, 3)) +
  xlab("") + 
  ylab(rownames(buccalCell_dfAge)[i]) +
  theme_classic())
}

grid.arrange(plot_AgeAccelerationResidual, plot_AgeAccelerationResidualHannum, plot_AgeAccelPheno, plot_DNAmAgeSkinBloodClockAdjAge, plot_AgeAccelGrim, plot_IEAA, plot_EEAA, plot_DNAmTLAdjAge, nrow = 3) 
quartz.save(file = 'buccalCell_ageAccel_rainCloudPlots.png', type = 'png', dpi = 300)
```

#### Adjusted robust linear regression results
```{r, echo = F, out.width = '75%'}
knitr::include_graphics("buccalCell_ageAccelAssoc_forestPlots_robust.png")
```

#### Raincloud plots
```{r, echo = F, out.width = '100%'}
knitr::include_graphics("buccalCell_ageAccel_rainCloudPlots.png")
```
